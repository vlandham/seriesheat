...
export const fetchSearch = async (searchString: string) => {
  ...
  const results = await worker.db.query(`select t.primaryTitle, t.tconst from searchtitles t where t.primaryTitle match "${searchString}*";`)
  return results
}
...
export const fetchRatings = async (parentId: string) => {
  const worker = await loadWorker()
  const parentData = await worker.db.query(`select t.tconst, t.primaryTitle from searchtitles t where t.tconst = '${parentId}';`)
  let ratingsData = await worker.db.query(`SELECT e.tconst, e.seasonNumber, e.episodeNumber, r.averageRating, r.numVotes from episodes e  left join ratings r on e.tconst = r.tconst where e.parentTconst = '${parentId}'; `) as Episode[]
  let output: any = {}
  ...
  if (ratingsData) {
    ...
    ratingsData.forEach((episode: Episode) => {
      ...
      if (isNumeric(episode.seasonNumber as string)) {
        episode.seasonEpisode = (+episode.seasonNumber * 100) + (episode.episodeNumber as number)
      } else {
        episode.seasonEpisode = (episode.seasonNumber as string) + (episode.episodeNumber.toString())
      }
    })
    ratingsData.sort((a: Episode, b: Episode) => (a.seasonEpisode as any) - (b.seasonEpisode as any))
    ratingsData = ratingsData.filter(e => e.episodeNumber > 0)

    const byEpisode = nest<Episode, Episode>().key((d) => d.seasonNumber as string).key((d) => d.episodeNumber as string)
      .rollup((d) => {
        if (d.length > 1) {
